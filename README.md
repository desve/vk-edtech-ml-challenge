...
3. Положить в `FOLDER` файлы:

- `users.tsv`, `history.tsv`, `validate.tsv`, `validate_answers.tsv`;
- `metrics.py` (из условия задачи).

4. Запускать ноутбуки по порядку:

- `notebooks/01_eda.ipynb` — базовый EDA и проверка структуры данных (опционально).  
- `notebooks/02_feature_engineering.ipynb` — экспериментальные признаки и кеширование тяжёлых агрегаций (опционально).  
- `notebooks/03_modeling.ipynb` — построение `features_df`, обучение классификатора и стеков, генерация `preds_validate.parquet`.  
- `notebooks/04_submission_and_report.ipynb` — формирование `submission.tsv`, запуск `metrics.py`, создание `autogenerated_report.txt`.

После запуска 04‑го ноутбука значение SMLogAccRatio и краткий отчёт будут записаны в `autogenerated_report.txt` в папке `FOLDER`.

---

## Для чего этот репозиторий

- Демонстрация **полного ML‑пайплайна** для промышленной задачи прогноза эффективности рекламы: от сырых TSV‑файлов до метрики качества.  
- Пример сочетания:
- аккуратного **feature engineering** (демография + история показов + временные агрегаты);  
- **ансамблей моделей** (RandomForest + LightGBM + HGB + Ridge);  
- **калибровки предсказаний** (Isotonic Regression);  
- и автоматической оценки/репорта (`metrics.py` + AutoReport).

Решение фокусируется на воспроизводимости и прозрачной структуре — его можно использовать как базовый шаблон для похожих задач (CTR/CVR‑прогноз, uplift‑моделирование, scoring рекламных кампаний).
...
- GitHub: [desve](https://github.com/desve)  
- Другой ML‑проект: [VK Text‑to‑Image Demo](https://github.com/desve/vk-text-to-image-demo) — генерация изображений по текстовому описанию с использованием диффузионной модели.
